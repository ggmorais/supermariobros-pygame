...
class Tileset:
    ...
    def get_tile(self, pos: int, scale: bool = True):
        tile = self.tiles[pos]
        rect = tile.get_rect()
        return tile if not scale else pg.transform.scale(tile, (rect.w * SCALE, rect.h * SCALE))
    ...
    def load(self):
        ...
        for y in range(y0, h, dy):
            for x in range(x0, w, dx):
                tile = pg.Surface(self.size, pg.SRCALPHA)
